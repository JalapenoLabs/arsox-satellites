...
| The route closure, applied by the satellite | deployment config today | Two `iptables` rules in a container run with `--cap-add NET_ADMIN`, so the agent account reaches loopback and nothing else. The recipe is documented and shipped as `scripts/close-the-route.sh`; the satellite applying it at boot needs `iptables` in the image, an explicit opt-in, and a privileged container smoke to assert it. |
...
**The enforcing deployment closes the route around the egress proxy, and that is configuration rather than code.** A process inside a container cannot remove its own default route without `NET_ADMIN`, and a satellite holding `NET_ADMIN` for its whole life would be a larger thing than the gate it raises. So the recommended deployment runs the container with `--cap-add NET_ADMIN` and applies two rules before the satellite starts: accept everything on loopback, refuse everything else from the agent account. The proxies listen on loopback, so agents keep what they are meant to have; the satellite is root, so its own model requests, clones, and `gh` polling are untouched. The recipe is [`scripts/close-the-route.sh`](./scripts/close-the-route.sh) and the reasoning, including the option for a deployment that will not grant the capability and what that option genuinely costs, is in [Deterministic enforcement](./docs/enforcement.md#the-route-closure-is-deployment-configuration).
...
**The browser goes through the egress proxy like everything else.** A browser that can reach any host would be a hole straight through the [web permissions](#permissions) model, since "navigate to a URL" is a network request wearing a different hat. Same allowlist, same enforcement. It gets there by the same mechanism every other tool does: headless Chrome reads `HTTP_PROXY` and `HTTPS_PROXY` out of the environment it is launched with, and a browser's navigation is a `CONNECT` naming its host exactly as `curl` is.
